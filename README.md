...
Scheduled dates and wall-clock times use **America/Los_Angeles**. There is no Google Calendar synchronization or automated recurring-event expansion. Navigation loads the selected calendar month, and the client calendar also includes workout logs, nutrition entries, and weekly reflections. Existing weekly check-ins retain their UTC Monday–Sunday grouping; their calendar marker is the week-start date.
